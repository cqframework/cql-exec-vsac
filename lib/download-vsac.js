...
function vsacAuthWarning(oldFnName, newFnName) {
  console.warn("WARNING! As of Jan 1 2021 VSAC will no longer accept accept username and password.  As such "
    + oldFnName + " has been deprecated, please use the new '" + newFnName + "' function instead!");
}
...
function downloadFromVSAC(username, password, input, output, vsDB={}, caching=true) {
  //deprecation warning
  vsacAuthWarning("downloadFromVSAC", "downloadFromVSACWithAPIKey");
...
function getTicketGrantingTicket(username, password) {
  //deprecation warning
  vsacAuthWarning("getTicketGrantingTicket", "getTicketGrantingTicketWithAPIKey");
  debug('Getting TGT');
  const options = {
    method: 'POST',
